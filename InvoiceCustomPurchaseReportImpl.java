...
@Service
@Slf4j
public class InvoiceCustomPurchaseReportImpl {
	...
	public InvoiceCustomPurchaseReportImpl(ResponseUtility respUtil, Utility utility,
	...
	public JSONObject generateCustomPurchaseRegisterReport(List<EinvReportGridDto> einvReportDtoObj, ReportRequest reqData,String reportFileName) throws IOException{
		log.info("Inside generateCustomPurchaseRegisterReport() of InvoiceCustomPurchaseReportImpl.java : START ");
		
		JSONObject responseJson = new JSONObject();
		String filePath = null;		

		filePath = einvReportDownloadFilePath+reportFileName;
		log.info("Inside generateCustomPurchaseRegisterReport() of InvoiceCustomPurchaseReportImpl.java : filepath is ::{} ", filePath);
		...
		try (FileOutputStream fs = new FileOutputStream(f)){			
			...
			if(einvReportDtoObj!=null) {							
				writeCustomPurchaseRegisterReportExcel(einvReportDtoObj,sheet,reqData);
				sheet.finish();	
				wb.finish();					
			}
			...
			log.error("Exception occurred in generateCustomPurchaseRegisterReport() of InvoiceCustomPurchaseReportImpl.java :: Exception {} ", e.toString());	
			throw e;
		}finally {			
			boolean deleteflag = Files.deleteIfExists(f.toPath());
			log.info("Local file copy deleted succesfully: flag={}", deleteflag);
		}
		log.info("Leaving generateCustomPurchaseRegisterReport() of InvoiceCustomPurchaseReportImpl.java : END ");
		return responseJson;
	}
	
	private void writeCustomPurchaseRegisterReportExcel(List<EinvReportGridDto> einvReportDtoObjList, Worksheet sheet, ReportRequest reqData) {
		log.info("Inside writeCustomPurchaseRegisterReportExcel() of InvoiceCustomPurchaseReportImpl.java : START ");
		...
		Set<String> fieldsAlignmentList = new HashSet<>(Arrays.asList("unitPrice", "subTotal","discountRate","discountAmount","feeOrChargeRate","feeOrChargeAmount","itemExcludingTax","itemTaxableValue","taxRate","taxAmount","itemIncludingTax",
       		 ...
       		 "totalTaxAmount","totalIncludingTax","prePaymentAmount","roundingAmount","totalPayableAmount","docLevelAmountExemptedFromTax"));
		...
		try {
			...
			if (reqData.getReportName().equalsIgnoreCase(DataConstants.CUSTOMPURCHASEREGISTERERRORREPORT)) {
					...
					for(ReportsCustomColumnsDTO reportsCustomColumnsDTO : reportsCustomColumnsDTOList ) {
				             sheet.value(startRow,columnNo++ ,checkNullValue(customTemplateValueMapList.get(i).get(reportsCustomColumnsDTO.getField())));
								if (fieldsAlignment.contains(reportsCustomColumnsDTO.getTitle().toLowerCase())) {
									sheet.style(startRow, columnNo)
											.horizontalAlignment(ReportsConstants.RIGHT_ALIGNMENT).set();
								}
					}
					startRow++;
			     }
			...
			log.error("Exception occurred in writeCustomPurchaseRegisterReportExcel() of InvoiceCustomPurchaseReportImpl.java :: Exception {} ", e.toString());	
			throw e;
		}

		log.info("Leaving writeCustomPurchaseRegisterReportExcel() of InvoiceCustomPurchaseReportImpl.java : END ");
	}
